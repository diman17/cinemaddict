...
const renderFilms = (films, container, from, to, onDataChange, onShowFilmDetails) => {
  return films.slice(from, to).map((film) => {
    const filmController = new FilmController(container, onDataChange, onShowFilmDetails);
...
export class PageController {
  ...
  _renderExtraFilms(component, sorting) {
    ...
    renderFilms(extraFilms, component.getElement().querySelector('.films-list__container'), 0, extraFilms.length, this._onDataChange, this._onShowFilmDetails);
  }
  ...
  _handleShowMoreButton() {
    ...
    const newFilms = renderFilms(sortedFilms, this._filmListContainer, this._prevFilmsCount, this._showingFilmsCount, this._onDataChange, this._onShowFilmDetails);
    this._showedFilmControllers = this._showedFilmControllers.concat(newFilms);
  ...
  _handleSortTypeChange(currentSortType) {
    ...
    const newFilms = renderFilms(sortedFilms, this._filmListContainer, 0, this._showingFilmsCount, this._onDataChange, this._onShowFilmDetails);
